...
Connect-AzAccount -TenantId xxxx-yyy-zzzzz # Adicionar seu tenant

# Coloque na variável abaixo o endereço do proxy que deseja utilizar para o Azure Monitor Agent
$ProxyAddr = " http://meuproxycustomizado:8080" # Proxy 
$ExtensionName = "AzureMonitorLinuxAgent-proxycustom"


function CheckCurrentExtensions {
    param ([string]$MachineName,[string]$ResourceGroup)
    $AMALinuxExtensions = Get-AzConnectedMachineExtension -MachineName $MachineName -ResourceGroupName $ResourceGroup | ? {$_.MachineExtensionType -eq "AzureMonitorLinuxAgent"} | select Name, ProvisioningState
    if ($AMALinuxExtensions)
    {
        Write-Host
...
{   
    clear-host
    write-host "======== Selecione as opções de escolha de servidores ARC Linux ========" -ForegroundColor Green
    write-host "1. Todos os servidores com agente ARC Linux"
    write-host "2. Escolher os servidores os servidores com agente ARC Linux"
    write-host "3. Sair"
    write-host
    $choice = Read-Host
} 
...
foreach ($ARCServer in $ARCServers)
{
    ...
    $CheckForExtension | % {if ($_.MachineExtensionType -eq "AzureMonitorLinuxAgent" -and $_.Name -eq $ExtensionName -and $_.ProvisioningState -eq "Succeeded") {$ExtensionInstalled = $true}}
    if (!($ExtensionInstalled)) 
    {
       if ($CurrentServer.Properties.osName -eq "Linux")
       {
        ...
        { 
            write-host "Extensão não encontrada - instalando... Servidor " $CurrentServer.name -ForegroundColor Green
                ...
                New-AzConnectedMachineExtension -Name $ExtensionName -ExtensionType AzureMonitorLinuxAgent -Publisher Microsoft.Azure.Monitor -ResourceGroupName $rg -MachineName $machine -Location $location -Setting $ProxySettings -NoWait
                $instalados++
            }
            else 
            {
            ...
            write-host "O servidor " $CurrentServer.Name "não é uma máquina executando Linux. Instalação não aplicável" -ForegroundColor Yellow
        }      
