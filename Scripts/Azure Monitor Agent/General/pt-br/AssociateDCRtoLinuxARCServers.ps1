# =================================================================================
# AssociateDCRtoLinuxARCServers.ps1
#
...
$FullLinuxARCServerList = get-azresource -ResourceType microsoft.HybridCompute/machines -ExpandProperties | ? {$_.Properties.osName -eq "linux"}
...
switch($choice)
{
    "1" 
    {
        $ArcServersList = $FullLinuxARCServerList
    }
    "2"
    {
        write-host "Selecione os servidores na janela que vai se abrir"
        $ArcServersList = $FullLinuxARCServerList | Out-GridView -PassThru -Title "Selecione os servidores (um ou mais) e clique em ok"
    }
    "3"
    {
        exit
